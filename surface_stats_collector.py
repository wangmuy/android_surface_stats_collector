...
class SurfaceStatsCollector(object):
  ...
  def __init__(self, device, layerName='SurfaceView', forceLegacyMethod=False):
    # TODO(jbudorick) Remove once telemetry gets switched over.
    #if isinstance(device, android_commands.AndroidCommands):
    #  device = device_utils.DeviceUtils(device)
    self._device = device
    self._layer_name = layerName
    self._force_legacy_method = forceLegacyMethod
  ...
  def Start(self):
    ...
    if self._ClearSurfaceFlingerLatencyData() and (not self._force_legacy_method):
  ...
  def _CollectorThread(self):
    ...
    while not self._stop_event.is_set():
      ...
      try:
        ...
        #print '_CollectorThread:'+'new_timestamps='+str(new_timestamps)+', timestamps='+str(timestamps)
  ...
  def _ClearSurfaceFlingerLatencyData(self):
    ...
    results = self._device.RunShellCommand(
        'dumpsys SurfaceFlinger --latency-clear '+self._layer_name)
  ...
  def _GetSurfaceFlingerFrameData(self):
    ...
    results = self._device.RunShellCommand(
        'dumpsys SurfaceFlinger --latency '+self._layer_name)
    if not len(results):
      return (None, None)
